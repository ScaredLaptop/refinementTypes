...
(require rackunit)
(require redex)
(require "../branchesRecursion.rkt")


; (test-true
;     "syntax 2"
;     (redex-match? TypedLambda/Recursion e
;                   (term (rec f = ((λ (x) (if y then x else (f z))) : ((x : (Int {v : true})) -> (Int {v : true}))) in (f a)))))

; (test-true
;      "syntax 3"
;         (redex-match? 
;         TypedLambda/Recursion e
;         (term (let (x = true) in 
...
;     (redex-match? 
;         TypedLambda/Recursion e
;         (term (λ (x) (if x then false else true)))
;     )
; )
;  (gen-racket-prim-op-expr 'add)
; (test-true
;     "MORE STUPID SYNTAX"
;     (redex-match? 
;         TypedLambda/Recursion t
;         (term ((x : (Bool {v : true})) -> (Bool {v : (= v (not x))})))
;     )
; )
...
;            (redex-match? TypedLambda/Recursion b (term Int)))

; (test-true "constants"
;            (redex-match? TypedLambda/Recursion constants (term 1)))
; (test-true "constants"
;            (redex-match? TypedLambda/Recursion constants (term mul)))

; (test-true "refinements"
;            (redex-match? TypedLambda/Recursion r (term {v : (and 0 x)})))

; (test-true "types"
;            (redex-match? TypedLambda/Recursion t (term (Int {v : (< 0 v)}))))

; (test-true "typed var"
;            (redex-match? TypedLambda/Recursion (x : t) (term (x : (Int {v : (< 0 v)}))))
;            )
; (test-true "fxn type"
;            (redex-match? TypedLambda/Recursion t (term ((a : (Int {v : (< 0 a)})) -> (Int {v : (< 0 a)}))))
;            )

; (test-true "nested fxn type"
;            (redex-match? TypedLambda/Recursion t (term ((a : (Int {v : (< 0 a)})) -> ((a : (Int {v : (< 0 a)})) -> (Int {v : (< 0 a)})))))
;            )

(define (≈? a b) (alpha-equivalent? TypedLambda/Recursion a b))
...
(test-true "Typing 1"
           (redex-match? TypedLambda/Recursion Γ (term (x : Int •)))
           )
(test-equal? "Typing 2"
           (term (extend • x Int))
           (term (x : Int •))
...
(test-true "Broken 1"
           (redex-match? TypedLambda/Recursion c (term (forall (x Int) (implies (and x y) (and y x)))))
           )
(test-true "Broken 2"
           (redex-match? TypedLambda/Recursion (b (x : p)) (term (Int {x : (< 0 x)})))
           )
(test-true "Broken 3"
           (redex-match? TypedLambda/Recursion (x (b (v : p))) (term (x (Int {x : (< 0 x)}))))
           )
(test-true "Broken 4"
           (redex-match? TypedLambda/Recursion (Γ x (b (v : p))) (term (• x (Int {x : (< 0 x)}))))
           )
(test-true
    "Broken 5"
  (redex-match? TypedLambda/Recursion
  (y : (b {y : p}) Γ)
  (term (extend (extend • x (Int (x : (< x 0)))) y (Int (y : (= y (+ x 1)))) ))
  )
)
...
(test-true "l" (redex-match? TypedLambda/Recursion
  t (term ((x1 : Int)
    ->                        
    (Int {y : (= y (+ x1 1))})))))
